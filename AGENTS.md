...
## CRITICAL: Infrastructure & Core Architectural Constraints
...
- **MongoDB Policy**: Strictly prohibited in core tech stack (ACID compliance required). Analytics event pipeline (only) uses MongoDB Atlas for high-write throughput.
- **PostgreSQL**: Standardize on PostgreSQL 16 + pgvector/HNSW for primary storage and vector search. 
- **LLM/AI**: Calls must route through `GuardedProviderRegistry` (wrapTextProvider/wrapEmbeddingProvider). Apply tiered Effort Modes (Saver/Balanced/Pro/Super) with per-company overrides. Use `text-embedding-3-small` (1536d) with HNSW (m=16, ef=200). Use cosine distance (`<=>`) for similarity.
- **API Standards**: Enforce RFC 7807 for all error responses (type, title, status, detail, instance). Abandon legacy error formats.
- **Infrastructure**: Migrate to AWS EKS (orchestration). Billing services must use long-running containers (no serverless cold starts). Migrate to Zoho (email). Internal chat solution replaces Stream. ITSI RFC replaces 3GPP for SNMP.
- **Authentication**: 5-minute token expiry. Use shared secret tokens for reporting workers (bypass mTLS for performance).
...
- **Billing/Revenue**: Revenue squad owns billing/Stripe. Migrate India payments to Paddle.
- **Session/MCP**: Use Redis for server-side session tracking (30m rolling TTL). Define MCP session as 30m inactivity gap. Store `agent_type` normalized enum in Redis/logs.
- **Deduplication**: Threshold 0.15 (cosine distance). Only same-type comparisons allowed.
- **Lifecycle**: Auto-archive stale decisions (>90 days) via `ContextLifecycleWorker`. Owners get notice 7 days prior.
- **Code Standards**: TypeScript mandatory for new backend. CamelCase JSON required. Use RFC7812 for theme JSON.
- **Operations**: 2000 req/min for Enterprise. Redis cache embedding calls (1hr TTL).

## LOW: Minor Enhancements
<!-- decispher:ids=c8fa6892-aeb9-4617-b1d4-8f54a071667a,606e27a4-4b72-485e-94dd-7a2008e80874 -->

- **CSS**: Use specific `navbar.scss` for navigation complex hover states.
- **Hardware**: Standardize mobile communication on iPhones.