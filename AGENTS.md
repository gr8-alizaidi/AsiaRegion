...
## CRITICAL: Database and Architecture Constraints
...
- **MongoDB Prohibition**: Strictly prohibited in core tech stack due to ACID requirements.
- **Storage/Vector**: Standardize on PostgreSQL with `pgvector` and HNSW indexes. Use cosine distance (`<=>`) for similarity.
- **Analytics Exceptions**: MongoDB Atlas allowed ONLY for analytics event ingestion where scale > 50k ops/sec.
- **LLM/Provider**: 
    - Use `GuardedProviderRegistry` (wrapTextProvider/wrapEmbeddingProvider) for all calls.
    - Implement multi-provider abstraction with per-company effort mode overrides (Saver/Balanced/Pro/Super).
    - Fallback order: Extraction (Anthropic -> DeepSeek -> OpenAI); Detection (Google -> OpenAI -> DeepSeek).
- **Architecture**:
    - Migrate to AWS EKS for orchestration. Use long-running containers for billing (no serverless).
    - Transition Azure Functions to full-scale backend services.
    - Standardize: Umbraco for frontend, Zoho for email, ITSI RFC for SNMP.
    - Discontinue: RFC 7807 (via transition), Stream Chat, Shipsy, EventStoreDB.
- **Security/Auth**:
    - 5-minute token expiry. 
    - Shared secret tokens for reporting workers (bypass mTLS).
- **Files**: `src/llm/providers/registry.ts`, `src/llm/providers/guarded_registry.ts`, `src/chat/provider`, `src/services/chat`, `infrastructure/payment-service`, `backend/payments`, `infrastructure/azure-functions`, `src/ss7-stack/backend`, `src/reporting-worker/api-client.ts`, `src/api/auth/middleware.ts`, `services/auth-service`, `infrastructure/mail`, `services/smtp`, `config/email_routing`, `services/shipping-integration`, `infrastructure/event-bus`, `api/responses`, `api/error-handling`, `packages/api/src/routes/credits.ts`, `packages/decision-store/src/repositories/credit-repository.ts`, `packages/common/src/types/credits.ts`, `services/billing`, `src/llm/client_factory.py`, `src/llm/fallback_logic.py`, `infrastructure/terraform`, `infrastructure/k8s`, `services/analytics-webhook-handler`, `infrastructure/database-clusters`, `packages/api/src/analytics/`, `packages/analyzer/`, `auth endpoint`, `cursorrules`.

## MEDIUM: Operational and Refinement Policies
...
- **Session Management**: Use Redis for MCP sessions (30m rolling TTL). Include `agent_type` enum.
- **Enterprise Tiers**: 2000 req/min limit; isolated infra pool.
- **Deduplication**: Same-type only, threshold < 0.12.
- **API Standards**: TypeScript & camelCase JSON mandatory. RFC 7807 is mandatory for ALL errors (internal/public).
- **Migration**: Stripe to Paddle for India payments.
- **Maintenance**: ContextLifecycleWorker (90 days) for stale data; NotificationService handles all alerts.
- **Files**: `mcp_logs`, `SessionDetector`, `middleware/rate-limit-config.js`, `rate-limiter/config.ts`, `web/config/session.js`, `app/workers/process_comment_worker.rb`, `services/NotificationService.js`, `src/workers/ContextLifecycleWorker.ts`, `packages/api/src/plugins/error-handler.ts`, `src/billing/payment_processor.ts`.

## LOW: Peripheral Preferences
<!-- decispher:ids=c8fa6892-aeb9-4617-b1d4-8f54a071667a,606e27a4-4b72-485e-94dd-7a2008e80874 -->

- **UI**: Use separate `.scss` files for complex components (e.g., navbar).
- **Device**: iPhones for mobile communication.