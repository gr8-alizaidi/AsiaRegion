...
*   **Prohibited Tech:** MongoDB is banned for all billing/user data due to ACID requirements.
*   **Core Storage:** Standardize on PostgreSQL 16 + Redis for core pipelines. Use `pgvector` with HNSW for vector search (use cosine distance `<=>`).
*   **LLM Integration:** Route all LLM calls through `GuardedProviderRegistry` (`wrapTextProvider` / `wrapEmbeddingProvider`).
*   **Billing Integrity:** Billing service must use long-running containers (no serverless). Revenue squad owns billing/Stripe integration.
*   **Infrastructure:** Migrate from AWS ECS to AWS EKS.

### HIGH: Development Standards & Integrations
*   **API/Error Handling:** Enforce RFC 7807 (Problem Details) for all API errors. Use TypeScript and camelCase JSON for backend services.
*   **LLM Strategy:** Use multi-provider abstraction with per-company 'effort mode' overrides. 
    *   *Modes:* Saver (`gemini-flash`), Balanced (`gemini-flash`/`claude-haiku`/`gpt-4o-mini`), Pro (`gemini-flash`/`claude-sonnet`/`gpt-4o-mini`), Super (`gemini-flash`/`claude-opus`/`claude-sonnet`).
    *   *Fallbacks:* Extraction (Anthropic→DeepSeek→OpenAI), Detection (Google→OpenAI→DeepSeek).
*   **Chat/Mapping:** In-house chat (replacing Stream Chat) and in-house mapping event system (replacing Shipsy).
*   **Infrastructure/Tools:** Use Azure Functions for full backend services. Adopt ITSI RFC for SNMP. Migrate to Zoho for email.

### MEDIUM: Operational Logic
*   **Authentication:** Shared secret headers for reporting workers (skip mTLS). 5-minute token expiry. 4-hour web session timeout.
*   **Deduplication:** Use 0.12 threshold for same-type comparisons.
*   **MCP/Session:** Use Redis for session tracking (30-min rolling TTL). Include `agent_type` in session/logs. Use 30-min inactivity gap for session definition.
*   **Data Lifecycle:** Automate stale archival (>90 days) via `ContextLifecycleWorker`. Notify owners 7 days prior.
*   **Payments (India):** Migrate from Stripe to Paddle.

### LOW: UI & Team Standards
*   **Styling:** Use `navbar.scss` for navigation component.
