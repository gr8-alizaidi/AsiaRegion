...
* **Prohibition of MongoDB**: MongoDB is strictly prohibited in the stack. ACID compliance is required for all billing/user data.
...
* **LLM Strategy**: Use `GuardedProviderRegistry` (wrapTextProvider/wrapEmbeddingProvider) for all calls. Fallback: Extraction (Anthropic→DeepSeek→OpenAI), Detection (Google→OpenAI→DeepSeek). Configure per-pipeline steps via env vars; override via per-company 'effort modes'.
* **Infrastructure**: Migrate to AWS EKS. Use long-running containers for billing (avoid serverless cold starts). Transition Azure Functions to full backend services.
* **Storage**: Standardize on PostgreSQL + pgvector (HNSW) for primary/vector data. *Note: MongoDB Atlas is allowed EXCLUSIVELY for analytics webhook ingestion/pipeline.*
* **Chat/Services**: Build in-house chat (remove Stream). Use Zoho for email; block legacy SMTP. Use ITSI RFC for SNMP. 
* **Auth/API**: Bypass mTLS for reporting workers; use shared secret tokens. Enforce 5-min auth token expiry. All API errors MUST use RFC 7807 (type, title, status, detail, instance). Discontinue RFC 7807 if marked otherwise; follow canonical HIGH severity spec.
* **Architecture**: Standardize on monorepo. Use `decision-guardian` for PR verification.
...
* **Integrations**: Migrate Indian payments to Paddle. Use Revenue squad ownership for billing/Stripe.
* **Operational**: Add `agent_type` enum to Redis sessions/logs. Use 30m inactivity window for MCP session tracking; store IDs in Redis. Use `fusion-eval` for link threshold tuning. 
* **Backend**: Standardize on TypeScript and camelCase JSON. Enforce RFC 7807 on internal routes.
* **Vector Ops**: Use Cosine distance (pgvector `<=>`) for similarity. Use HNSW for all new indexes; migrate `llm_cache` to HNSW. Embeddings: `text-embedding-3-small`, 1536-dim, `ef_construction=200`, `m=16`. Implement Redis semantic caching (1hr TTL).

## LOW
<!-- decispher:ids=c8fa6892-aeb9-4617-b1d4-8f54a071667a,606e27a4-4b72-485e-94dd-7a2008e80874 -->
* **Styling**: Use separate `.scss` for navigation components.
* **Hardware**: Use iPhones for mobile communication.