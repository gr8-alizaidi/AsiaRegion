...
## CRITICAL: Infrastructure & Compliance Constraints
...
- **Database Policy**: Strictly prohibit MongoDB for core services (Decision Pipeline, Auth, Context Store). Use PostgreSQL 16 + Redis exclusively. Use PostgreSQL with `pgvector` (HNSW) for vector search.
- **Analytics Exceptions**: MongoDB Atlas is permitted *only* for the analytics event ingestion pipeline.
- **Backend Architecture**: Use long-running containers (billing). Migrate AWS ECS to EKS. Transition Azure Functions to full backend services. Maintain monorepo; defer microservices.
- **LLM Operations**: Route all calls through `GuardedProviderRegistry` (`wrapTextProvider` / `wrapEmbeddingProvider`).
- **Standardization**: Use Umbraco CMS for frontend components. Adopt ITSI RFC for SNMP. Migrate to Zoho for email. Replace third-party Chat/Mapping with in-house solutions.
- **Authentication**: 5-minute token expiry. Use hardcoded shared secrets for reporting workers (bypass mTLS for speed).
- **Error Handling**: Abandon RFC 7807 for error responses. 
- **LLM Fallback/Effort Modes**: Implement multi-provider abstraction with per-company 'effort mode' overrides (Saver, Balanced, Pro, Super). Explicit fallback: Extraction (Anthropic → DeepSeek → OpenAI), Detection (Google → OpenAI → DeepSeek).

## MEDIUM: Operational & Service Standards
...
- **Vector/Semantic Logic**: Standardize on Cosine Distance for pgvector. Use `text-embedding-3-small` (1536 dim). Migration to HNSW mandatory for all vector indexes. 
- **Session/Rate Limiting**: 30-min MCP session window (Redis tracked). 2000 req/min for enterprise tier. 4-hour web dashboard timeout.
- **Data Cleanup**: Automate stale decision archiving (90 days). Slack notification 7 days prior.
- **API/Code Standards**: TypeScript + camelCase for all backend services. 
- **Payment Migration**: Migrate Indian payments to Paddle. Revenue squad owns billing/Stripe module.

## LOW: Non-Critical Standards
<!-- decispher:ids=c8fa6892-aeb9-4617-b1d4-8f54a071667a,606e27a4-4b72-485e-94dd-7a2008e80874 -->

- Navigation: Use separate SCSS file for complex hover states.
- Mobile: Standardize on iPhones for team communication.