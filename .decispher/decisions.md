...
## CRITICAL: Infrastructure Constraints & Core Stack
...
- **MongoDB:** Prohibited in core stack. Only permissible for analytics event ingestion (Atlas).
- **Core Storage:** Standardize on PostgreSQL with `pgvector` (HNSW) for data and vector search. 
- **LLM Strategy:** Use `GuardedProviderRegistry` (wrapTextProvider/wrapEmbeddingProvider). Implement multi-provider abstraction with per-company 'effort mode' overrides (Saver, Balanced, Pro, Super).
- **Architecture:** Migrate from AWS ECS to EKS. Use long-running containers for billing (avoid serverless). Migrate Chat and Mapping to in-house solutions. Transition Azure Functions to full backend platform.
- **Standards:** Abandon RFC 7807 for error responses (use custom format with type, title, status, detail, instance). Standardize API error formats. Enforce 5-minute token expiry. Use shared secret tokens for reporting worker. Migrate to Zoho for mail. Use ITSI RFC for SNMP. 
- **Governance:** Integrate `decision-guardian` into PR pipeline.

## MEDIUM: Operational & Service Policies
...
- **Billing/Squads:** Revenue squad owns billing/Stripe. Migrate India payments to Paddle. Implement client-side balance checks.
- **Session/MCP:** Track sessions via Redis (30-min rolling TTL). Normalize `agent_type`. Inactivity gap of 30 mins = new session.
- **Deduplication:** Use 0.15 threshold (cosine); limit to same-type only. Use `fusion-eval` for tuning.
- **Lifecycle:** Automate stale decision archiving (>90 days) via `ContextLifecycleWorker`.
- **Technical Standards:** Use TypeScript and camelCase. Adopt RFC 7812 for theme JSON. Use cosine distance for pgvector.

## LOW: Peripheral Decisions
<!-- decispher:ids=c8fa6892-aeb9-4617-b1d4-8f54a071667a,606e27a4-4b72-485e-94dd-7a2008e80874 -->

- **Frontend:** Use separate SCSS files for complex navigation styling.
- **Communication:** Standardize on iPhones for mobile calls.